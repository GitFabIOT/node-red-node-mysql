...
module.exports = function(RED) {
    ...
    function MySQLNode(n) {
        ...
        node.connectionPool = mysqldb.createPool({
            host: node.host,
            port: node.port,
        ...
        if (this.mydbConfig) {
            ...
            node.on('input', function(msg) {
                if (typeof msg.topic === 'string') {
                    ...
                    node.mydbConfig.getConnectionPool().getConnection(function(err, connection) {
                        if (err) {
                            node.error(err, msg);
                        } else {
                            connection.query(msg.topic, bind, function(err, rows) {
                                try {
                                    connection.release();
                                } catch (e) {
                                    console.log('connection.release err', e);
                                }
